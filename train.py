...
class TrainPipeline():
    ...
    def run(self):
        ...
        try:
            print("HAHA")
            print("%s Start Running"%time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()))
            for i in range(self.game_batch_num):
                ...
                if (i+1) % self.save_freq == 0:
                    self.policy_value_net.save_model('./current_policy_%d_%d.model'%(self.board_width,self.board_height))
                    with open("data_buffer.pkl","wb") as f:
                        pickle.dump(self.data_buffer,f)
                        print("Dump data, size = %d"%len(self.data_buffer))

                if (i+1) % self.check_freq == 0:
                    print("{} current self-play batch: {}".format(time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()),i+1))
                    win_ratio = self.policy_evaluate()
                    if win_ratio > self.best_win_ratio:
                        print("New best policy!!!!!!!!")
                        self.best_win_ratio = win_ratio
                        # update the best_policy
                        self.policy_value_net.save_model('./best_policy_%d_%d.model'%(self.board_width,self.board_height))
